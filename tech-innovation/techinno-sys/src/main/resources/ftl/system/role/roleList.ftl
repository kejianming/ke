...
<div class="lenos-search">
  <div class="select">
    ...
    <button class="select-on layui-btn layui-btn-sm" data-type="select"><i class="layui-icon"></i>搜索
    </button>
...
<script>
  ...
  layui.use('table', function () {
    ...
    table.render({
      ...
      , cols: [[
        ...
        , {field: 'roleName',align:'center', title: '角色名称', width: '25%', sort: true}
        , {field: 'remark',align:'center', title: '角色描述', width: '30%', sort: true}
        , {field: 'createDate',align:'center', title: '创建时间', width: '30%',templet: '<div>{{ layui.laytpl.toDateString(d.createDate,"yyyy-MM-dd") }}</div>'}
        , {field: 'remark', title: '操作', width: '20%',align:'center', toolbar: "#toolBar"}
      ]]
      , page: true
      ,  height: 'full-83'
    });
  ...
  function del(id) {
    $.ajax({
      ...
      success: function (d) {
    	if(d.flag){
          layer.msg(d.msg,{icon:6,offset: 'auto',area:['150px','100px'],anim:2});
          layui.table.reload('roleList');
        }else{
          layer.msg(d.msg,{icon:5,offset: 'auto',area:['150px','100px'],anim:2});
        }
      }
    });
  }
