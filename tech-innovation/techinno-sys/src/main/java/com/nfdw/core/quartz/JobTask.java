...
public class JobTask {
     ...
     * 开启
     */
    @Log(desc = "定时任务:开启定时任务")
    public boolean startJob(SysJob job) throws ClassCastException,ClassNotFoundException{
        Scheduler scheduler = schedulerFactoryBean.getScheduler();
        try {
     ...
     * 更新
     */
    @Log(desc = "定时任务:更新定时任务", type = LOG_TYPE.UPDATE)
     ...
     * 删除
     */
    @Log(desc = "定时任务:删除定时任务", type = LOG_TYPE.DEL)
