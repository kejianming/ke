...
import com.nfdw.config.ActPropertiesConfig;
...
//        editorNode.put("stencilset", stencilSetNode);
    ...
    @GetMapping(value = "goProcessInstance")
    public String goProcessInstanceList(org.springframework.ui.Model model, String deploymentId) {

        model.addAttribute("deploymentId", deploymentId);

        return "/actProeInstanList";
    }

    @GetMapping(value = "showProcessInstanceList")
    @ResponseBody
    public ReType showProcessInstanceList(org.springframework.ui.Model model, String deploymentId,
                                          String page, String limit) {

        if (StringUtils.isBlank(deploymentId) || StringUtils.isBlank(page) || StringUtils.isBlank(limit)) {
            return null;
        }
        List<ProcessInstance> processInstanceList = runtimeService.createProcessInstanceQuery().deploymentId(deploymentId)
        ...
        processInstanceList.forEach(processInstance -> list.add(new ProcessInstanceInfo(processInstance)));
    ...
    @GetMapping(value = "goTaskList")
    public String goTaskList(org.springframework.ui.Model model, String processInstanceId) {
    ...
    @ResponseBody
    public ReType showTask(org.springframework.ui.Model model, String processInstanceId,
                           String page, String limit) {

        if (StringUtils.isBlank(processInstanceId) || StringUtils.isBlank(page) || StringUtils.isBlank(limit)) {
            return null;
        }
        List<org.activiti.engine.task.Task> taskList = taskService.createTaskQuery().processInstanceId(processInstanceId)
                .listPage(Integer.valueOf(limit) * (Integer.valueOf(page) - 1), Integer.valueOf(limit));

        long count = taskService.createTaskQuery().processInstanceId(processInstanceId).count();

        List<Task> list = new ArrayList<>();
        taskList.forEach(task -> list.add(new Task(task)));
    ...
    @GetMapping(value = "goActStart")
    public String goActStart() {
        return "/act/startprocess/start";
    }
