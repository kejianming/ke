...
//            UserLeave userLeave = (UserLeave) variables.get("userLeave");
    ...
    @ResponseBody
    public String showTaskList(Model model, com.nfdw.entity.Task task, String page, String limit) {
        CurrentUser user = CommonUtil.getUser();
        ...
        List<com.nfdw.entity.Task> tasks = new ArrayList<>();
        Map<String, Object> map = new HashMap<>();
        com.nfdw.entity.Task taskEntity = null;
        ...
        for (Task task1 : taskList) {
            ...
            if (!(map.get("baseTask") instanceof BaseTask)) {
                continue;
            }
            BaseTask userLeave = (BaseTask) map.get("baseTask");

            taskEntity = new com.nfdw.entity.Task(task1);
...
//        BASE64Encoder encoder  = new BASE64Encoder();
    ...
    public InputStream generateStream(String processInstanceId, boolean needCurrent) {
        ...
        if (StringUtils.isEmpty(processDefinitionId)) {
            ...
            if (historicProcessInstance != null) {
                ...
                historicActivityInstanceList =
                        historyService.createHistoricActivityInstanceQuery().processInstanceId(processInstanceId).orderByHistoricActivityInstanceId().asc().list();
                for (HistoricActivityInstance activityInstance : historicActivityInstanceList) {
                    currentActivityIdList.add(activityInstance.getActivityId());
                }
            }
        }
        ...
        InputStream imageStream = processDiagramGenerator.generateDiagram(bpmnModel, "png"
                , currentActivityIdList, currentActivityIdList
                , processEngine.getProcessEngineConfiguration().getActivityFontName(), processEngine.getProcessEngineConfiguration().getLabelFontName()
                , "微软雅黑", null, 1);
        return imageStream;
    }
